...
var pahtExcel = 'D:/git/a3-client/配置表/language多语言.xlsx'; //  currentDirectoryPath.replace('Language', '') + 'Excels/Language_多语言表.xlsx';
var pathUI = currentDirectoryPath.replace('Language', '') + 'UI';
var pathAllExcel = 'D:/git/a3-client/配置表'; //currentDirectoryPath.replace('Language', '') + 'Excels';
var callBack = null;

// 脚本前缀
var prefix = 'LanguageManager.instance.getLanguage';
              ...
              if (matchArr) {
                ...
                for (let needChangeContext of matchArr) {
                  ...
                  if (!matches) {
                    ...
                    console.log('程序会先跳过这个文件,继续执行....', 'pick');
                    continue;
                  }
            ...
            fs.readFile(filePath, coding, async (err, data) => {
              ...
              let repCb = forData => {
                for (let key in forData) {
                  let cValue = forData[key]._string;
                  if (cValue) {
                    // 如果发现有中文
                    if (hasChineseCharacters(cValue)) {
                      // 如果表中有这个一样的内容，直接延用之前的内容
                      let result = checkLanguage(cValue);
                      let uiKey = '';
                      if (result) {
                        uiKey = result.key;
                      } else {
                        let isContentChange = false;
                        forData[key]._string = cValue.replace(/\r\n/g, '\n');
                        for (let id in outResult) {
                          if (outResult[id] === forData[key]._string) {
                        ...
                        if (!isContentChange) {
                          ...
                          uiKey = 'UTL_' + nextId;
                          outResult[nextId] = forData[key]._string;
                        }
                      }
                      forData[key]._string = uiKey;
                      isChange = true;
                    }
                  }
                }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let file of files) {
      ...
      if (stats.isFile()) {
        ...
        // 如果是文件夹，则递归调用函数继续读取文件夹内的文件
        await tryReplaceScript(filePath);
      }
    }
    resolve(true);
  });
...
function initLanguageId() {
  return new Promise((resolve, reject) => {
    workbook.xlsx
      ...
      .catch(err => {
        // 处理读取文件时出现的错误
        console.error('Error reading file 4:' + err);
        resolve(false);
      });
  });
}
  ...
  return new Promise((resolve, reject) => {
    workbook.xlsx
      ...
      .then(() => {
        ...
        let info = {};
        const sheetValues = worksheet.getSheetValues();
        for (let index = 3; index < sheetValues.length - 1; index++) {
        ...
        // 找到中文翻译的索引
        let chineseIndex = sheetValues[1].findIndex(value => value && value.indexOf('中文') !== -1);
        if (chineseIndex === -1) {
          console.error('没有找到中文备注，请检查！！！');
          return;
        }
        ...
        workbook.xlsx.writeFile(pahtExcel).then(() => {
          if (!isEnd) {
            console.log('写入表格成功，还没有结束，请等待。。。');
          } else {
            console.log('当前项目收集结束，等待其他项目收集', 'green');
          }
          resolve(true);
        });
      })
      .catch(err => {
        // 处理读取文件时出现的错误
        console.error('Error reading file 5:' + err);
        resolve(false);
      });
  });
}
...
function run(callBack) {
  try {
    ...
    pathZ = 'D:/git/a3-client/A3-Client/assets/scripts'; //currentDirectoryPath.replace('Language', '') + 'JavaScripts';
    pathAllExcel = 'D:/git/a3-client/配置表'; // excelPath.replace(/\\/g, '/');
    pahtExcel = pathAllExcel + '/' + 'language多语言.xlsx';
    pathUI = 'D:/git/a3-client/A3-Client/assets'; //currentDirectoryPath.replace('Language', '') + 'UI';
    ...
    workbook.xlsx
      ...
      .then(async () => {
        ...
        if (!fs.existsSync(pahtExcel)) {
          worksheet.addRow(['Language#1#1#Id', '唯一key', '中文', '英文']);
          worksheet.addRow(['id', 'name', 'value', 'value_e']);
        ...
        tryChangeUI(pathUI, collectUILanguage).then(async () => {
          console.log('UI 新收集的文本：' + JSON.stringify(collectUILanguage), 'blue');
...
function runMulti(
  ...
  try {
    ...
    tryReplaceScript(pathZ).then(async () => {
      console.log('替换代码完成', 'blue');

      tryReplaceTagExcel(pathAllExcel).then(async () => {
        console.log('替换tag完成', 'blue');
        callBack && callBack();
        callBack = null;
      });
    });
