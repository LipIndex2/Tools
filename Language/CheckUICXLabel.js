    ...
    for (let file of files) {
      ...
      if (stats.isFile()) {
        if (filePath.endsWith(".prefab")) {
          // 读取文件内容找到CXLabel的_wordCNKey为空的文件
          const arr = readToJson(filePath);
          for (let i = 0; i < arr.length; i++) {
            const info = arr[i];
            if (
              info.__type__ === "8c179saYmxKmI8EvC6upUob" &&
              !info._wordCNKey
            ) {
              if (!copyFilesPathInfo[file]) {
                copyFilesPathInfo[file] = [];
              }
              ...
              for (let j = 0; j < arr.length; j++) {
                ...
                if (
                  ...
                  arr[j + 1] &&
                  arr[j + 1].node &&
                  arr[j + 1].node.__id__ === info.node.__id__
                ) {
                  parent = v;
                }

                if (
                  (v.__type__ === "cc.Label" || v.__type__ === "cc.RichText") &&
                  v.node &&
                  v.node.__id__ === info.node.__id__
                ) {
                  if (!hasChineseCharacters(v._string)) {
                    parent = null;
                    continue;
                  } else {
                    111;
              ...
              if (parent) {
                copyFilesPathInfo[file].push(parent._name);
              } else {
                delete copyFilesPathInfo[file];
              }
            }
          }
        }
...
var rootPath = "D:/cocosProject/xb-client/xibu3/assets/resources/viewsNew";

// 先收集替换的文件路径
checkUICXLabel(rootPath, spineCollectPathInfo).then(async (result) => {
  let arr = [];

  const pathJson = "C:/Users/Public/Config/UnCentent.json";
